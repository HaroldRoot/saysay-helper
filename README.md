...
| 亚文化 | 简体 ↔ 火星文双向转换、Leet 1337 变体生成。 |
| 可爱符号 | 按主题分类的可爱符号面板，顶部 chip 切换分类，点击符号即复制。 |
...
- **响应式**：移动端浮动玻璃岛 8 个 tab 自动换行，桌面端展开为单行胶囊；不再使用 480px 强制窄列。
- **可读 tooltip**：问号下方紧贴 question-mark 弹出，最大 280px 自动换行；不再使用屏幕底部通栏，避免离触发点太远。
- **等宽字体取 JetBrains Mono**：`<code>` 与 logo 使用 `'JetBrains Mono', ui-monospace, SFMono-Regular, Menlo, Consolas, monospace`，其它正文继续走 Geist。

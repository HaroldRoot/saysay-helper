...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const zalgoLevel = document.getElementById('zalgo-level');
    ...
    function generateZalgo(text, level, up, mid, down) {
        ...
        return splitGraphemes(text).map(char => {
            ...
            const maxPerDir = Math.min(100, Math.floor(intensity * 100));

            if (up) {
                const numUp = Math.floor(Math.random() * maxPerDir);
                for (let i = 0; i < numUp; i++) {
                    newChar += getRandomChar(zalgoChars.up);
                }
            }

            if (mid) {
                const numMid = Math.floor(Math.random() * (maxPerDir / 2));
                for (let i = 0; i < numMid; i++) {
                    newChar += getRandomChar(zalgoChars.mid);
                }
            }

            if (down) {
                const numDown = Math.floor(Math.random() * maxPerDir);
                for (let i = 0; i < numDown; i++) {
                    newChar += getRandomChar(zalgoChars.down);
                }
            }
    ...
    function updateZalgoOutput() {
        ...
        const level = zalgoLevel.value;

        const transformedText = generateZalgo(
            text,
            level,
            zalgoUp.classList.contains("active"),
            zalgoMid.classList.contains("active"),
            zalgoDown.classList.contains("active")
        );
